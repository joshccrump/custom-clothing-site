// scripts/fetch-square.mjs (STRICT + Location fallback)
...
async function readLocationId() {
  ...
  try {
    ...
    const loc = (json && json.locationId || "").trim();
    if (loc) {
      console.log("ℹ️  Using Location ID from config/public-square.json");
      return loc;
    }
  } catch {}
  return "";
...
const LOCATION_ID = await readLocationId();
if (!LOCATION_ID) fail("Missing SQUARE_LOCATION_ID (env) and no config/public-square.json fallback found.");
...
async function verifyAuthAndLocation() {
  ...
  if (!ids.includes(LOCATION_ID)) {
    fail(`SQUARE_LOCATION_ID "${LOCATION_ID}" not found in ${ENV}. Available: ${ids.join(", ")}`);
  }
}
...
function indexById(objs) {
  const m = new Map();
  for (const o of objs || []) if (o && o.id) m.set(o.id, o);
  return m;
}
...
(async function main() {
  console.log("=== Square catalog sync (strict + fallback) ===");
