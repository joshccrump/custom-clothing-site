diff --git a/web/assets/gallery.js b/web/assets/gallery.js
index 1d696f279071d860a878fd4ed89d71c59c431027..1ba683445e6742a67149887450bf172c8b0519c1 100644
--- a/web/assets/gallery.js
+++ b/web/assets/gallery.js
@@ -37,79 +37,93 @@ function uniq(a) { return [...new Set((a || []).filter(Boolean))]; }
...
+    const hasVars=vars.length>0;
     const priceLine = hasVars?((p.price_min===p.price_max)?fmt(p.price_min,cur):`${fmt(p.price_min,cur)} – ${fmt(p.price_max,cur)}`):(typeof p.price==='number'?fmt(p.price,cur):'');
-    const oos = Number(p.stock||0)===0;
+    const oos = typeof p.stock==='number'?Number(p.stock)===0:false;
     const badges = []; if (oos) badges.push('<span class="badge out">Out of stock</span>'); if ((p.tags||[]).includes('best-seller')) badges.push('<span class="badge">Best Seller</span>');
...
+      for (const v of vars) {
         ...
         opt.value=v.id; const price = (typeof v.price==='number')?` — ${fmt(v.price,cur)}`:'';
...
+    }
 ...
 document.addEventListener('DOMContentLoaded', async function(){
   ...
   let ps=[];
   try { ps = await loadProducts(); } catch(e){ grid.innerHTML='<div class="subtle">Could not load products. Check data/products.json path.</div>'; console.error(e); return; }
   const cats=uniq(ps.map(p=>p.category)); catEl.innerHTML='<option value=\"\">All categories</option>'+cats.map(c=>`<option>${c}</option>`).join('');
   const sizes=uniq(ps.flatMap(p=>p.sizes||[])); sizeEl.innerHTML='<option value=\"\">Any size</option>'+sizes.map(v=>`<option>${v}</option>`).join('');
   function render(){ const filtered=filterProducts(ps,s); const {page,pages,items}=paginate(filtered,s.page,s.per); renderGrid(grid,items);
     pager.innerHTML=''; const prev=document.createElement('button'); prev.textContent='Prev'; prev.disabled=(page<=1); prev.onclick=()=>{s.page=Math.max(1,page-1);render()};
     ...
     const next=document.createElement('button'); next.textContent='Next'; next.disabled=(page>=pages); next.onclick=()=>{s.page=Math.min(pages,page+1);render()}; pager.append(prev,info,next);
   }
   searchEl.oninput=()=>{s.q=searchEl.value.trim();s.page=1;render()}; catEl.onchange=()=>{s.category=catEl.value.trim();s.page=1;render()}; sizeEl.onchange=()=>{s.size=sizeEl.value.trim();s.page=1;render()}; sortEl.onchange=()=>{s.sort=sortEl.value.trim();s.page=1;render()};
   render();
 });
