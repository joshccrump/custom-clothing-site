// Improved gallery: keeps Buy buttons even if variations live under different keys
(function(){
  ...
  function renderGrid(container, items) {
    ...
    for (const p of items) {
      ...
      if (vid) {
        buy.href = hrefFor(vid);
      } else {
        const viewHref = p.url || (p.slug ? (BASE + 'product.html?slug=' + encodeURIComponent(p.slug)) :
                         (p.id ? (BASE + 'product.html?id=' + encodeURIComponent(p.id)) : '#'));
        buy.href = viewHref;
      }
